  ...
  return <>
    ...
    <section className="eh-experience eh-experience--features" id="journey-workspace" data-chapter="workspace" aria-labelledby="features-title" tabIndex={-1}>
      <header className={`eh-experience__heading${feature ? ' eh-experience__heading--feature' : ''}`}>
        {feature ? <button type="button" className="eh-experience__back" onClick={() => choose(null)}><Icon.chevL size={15} /> Все возможности</button> : <p className="eh-journey__eyebrow">ФУНКЦИОНАЛ В ДЕЙСТВИИ</p>}
        <h2 id="features-title" ref={title} tabIndex={-1}>{feature ? feature.title : 'От записи клиента до консультации — всё под рукой'}</h2>
