...
function InfoRow({ icon = 'calendar', label, value }: { icon?: keyof typeof Icon; label: string; value: ReactNode }) {
  ...
  return <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 0', borderTop: '1px solid var(--line)' }}>
    ...
    <span style={{ fontSize: 14, color: 'var(--text-1)', fontWeight: 500, flex: 1, textAlign: 'right' }}>{value}</span>
  </div>
}
...
function ClientFragment({ step, setStep, guide }: FragmentProps) {
  ...
  return <div className="eh-source__narrow">
    ...
    {step === 2 && <>
      ...
      <div className="eh-source__stack">{shownNotes.map((text, index) => <div key={index} style={{ padding: '13px 15px', borderRadius: 13, border: '1px solid var(--line)', background: 'var(--surface-1)' }}><div style={{ display: 'flex', gap: 7, marginBottom: 6 }}><span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)' }}>Ваша заметка</span><span className="mono" style={{ fontSize: 12, color: 'var(--text-3)', marginLeft: 'auto' }}>{index === shownNotes.length - 1 ? '8 сентября' : 'только что'}</span></div><p style={{ margin: 0, fontSize: 14, lineHeight: 1.55 }}>{text}</p></div>)}</div>
    </>}
    {step === 2 && guide && <Notice show={saved}>Заметка сохранена</Notice>}
    {step === 3 && <>
      {/* app/crm-card.jsx → ClientInbox, original opposing bubble styles. */}
      <div className="eh-source__stack">{shownMessages.map((item, index) => <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: item.from === 'me' ? 'flex-end' : 'flex-start', gap: 3 }}><div style={{ maxWidth: '88%', padding: '10px 14px', borderRadius: 14, fontSize: 14, lineHeight: 1.45, background: item.from === 'me' ? 'var(--accent)' : 'var(--surface-2)', color: item.from === 'me' ? 'var(--text-on-accent)' : 'var(--text-1)', borderBottomRightRadius: item.from === 'me' ? 4 : 14, borderBottomLeftRadius: item.from === 'me' ? 14 : 4, border: item.from === 'me' ? 'none' : '1px solid var(--line)' }}>{item.text}</div><span className="mono eh-source__muted">Telegram · {item.time}</span></div>)}</div>
...
function ProductFragment({ step, setStep, guide }: FragmentProps) {
  ...
  return <div className="eh-source__narrow">{step === 0 ? <div className="card" style={{ overflow: 'hidden' }}>
    ...
    <div style={{ padding: '0 18px 16px', display: 'grid', gap: 8 }}>{['Натальная карта', 'Ответы на ваш запрос', 'Запись встречи и материалы'].map((text, index) => <div key={text} className="eh-source__reveal" style={{ ...reveal(guide, .15 + index * .16, .3 + index * .16), display: 'flex', gap: 8, fontSize: 14, color: 'var(--text-2)' }}><Icon.check size={13} />{text}</div>)}</div>
    <div style={{ padding: '12px 18px', borderTop: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 14 }}><span className="eh-source__muted">{guide ? "Готова к записи клиентов" : <>Продаж <b>14</b></>}</span>{!guide && <span style={{ color: 'var(--gold)', fontSize: 14 }}>68 600 ₽</span>}<button type="button" className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 14 }} onClick={() => setStep(1)}>Изменить</button></div>
